...
compile() {
    ...
    if [ "$1" = true ]; then
        ...
        cd code
        bash -x ./build_app.sh
        # g++ -o code src/SuperRes7.cpp `pkg-config --cflags --libs opencv4` for testing on local machine without Vitis AI
...
run_models() {
    ...
    rm -f outputs/memory_usage.csv
    rm -f outputs/execution_time.txt
    ./code/src/memory_footprint.sh &  # launch memory footprint in background
    pid_memory_footprint=$!

    echo "[SR7 INFO] Running CNN models"
    ./build/SuperRes7 $1 $2 $3 $4 $5 $6 $7 $8

    kill "$pid_memory_footprint"
    mv memory_usage.csv outputs/memory_usage.csv
    mv execution_times.txt outputs/execution_times.txt
    echo "[SR7 INFO] Saved memory footprint in outputs/memory_usage.csv"
    echo "[SR7 INFO] Saved execution times in outputs/execution_times.txt"
    echo " "
...
display_help() {
    ...
    echo "  -o,  --output-dir       Specify the output directory (default: outputs/)"
...
run_models $input_img $patch_size $stride $path_xmodel_supres $output_dir $num_threads $num_processes "debug/" 2>&1 | tee ${RUN_LOG}